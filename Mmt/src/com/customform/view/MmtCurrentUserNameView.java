 ...
 */
class MmtCurrentUserNameView extends MmtBaseView {

    MmtCurrentUserNameView(Context context, GDControl control) {
    ...
    public ImageEditView build() {
        ...
        if (control.Value.length() != 0) {
            ...
            view.setValue(control.DefaultValues.length() == 0 ? MyApplication.getInstance().getConfigValue("UserBean", UserBean.class).TrueName : control.DefaultValues);
        }
    ...
    protected View buildReadonlyView() {
        ...
        if (TextUtils.isEmpty(defaultValue)) {
            defaultValue = MyApplication.getInstance().getConfigValue("UserBean", UserBean.class).TrueName;
        }
