...
public class AddressSearchResultFragment extends Fragment {
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		adapter = new AddressSearchAdapter(getActivity(), TreeViewAdapter.PaddingLeft >> 1);
		List<AddressSearchAdapter.TreeNode> treeNode = adapter.GetTreeNode();
		for (int i = 0; i < parent.size(); i++) {
			AddressSearchAdapter.TreeNode node = new AddressSearchAdapter.TreeNode();
		...
		expandableListView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
				...
				AddressSearchResultCallback callback = new AddressSearchResultCallback(candidate);
	...
	private void initDate() {
		...
		for (LocatorGeocodeResult.Candidate candidate : LocatorGeocodeResult.geoResult.candidates) {
