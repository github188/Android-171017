...
public class Node {
    ...
    public static Node createSimpleTree(String rootName, List<String> childrenName, String checkedName) {
        Node root = new Node(rootName, rootName);
        for (String nodeName : childrenName) {
            Node child = new Node(nodeName, nodeName);
            child.parent = root;
            root.children.add(child);

            if (checkedName != null && nodeName.equals(checkedName)) {
                child.checked = true;
            }
        }
        return root;
